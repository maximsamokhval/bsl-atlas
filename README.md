...
**Поддерживаемые провайдеры эмбеддингов:** OpenAI, OpenRouter, Cohere, Jina AI, Ollama, Local, Deepinfra (новая опция).
**Поддерживаемые LLM провайдеры:** DeepSeek API (новая опция).

Для Deepinfra укажите `EMBEDDING_PROVIDER=deepinfra` и при необходимости `DEEPINFRA_API_KEY` (можно оставить пустым для анонимных запросов). Для DeepSeek укажите `DEEPSEEK_API_KEY` (обязательно) и `LLM_PROVIDER=deepseek` (если используется LLM-поиск).
